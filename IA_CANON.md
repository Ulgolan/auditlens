...
| A framework's identity — Nielsen's 10 | `Nielsen's 10` ↔ `NIELSEN'S 10 USABILITY HEURISTICS` | — | Register: chip label ↔ report heading, canonized as designed intent, both faces kept per framework. |
| A framework's identity — Cognitive Walkthrough | `Cognitive Walkthrough` ↔ `COGNITIVE WALKTHROUGH` | — | Register: chip label ↔ report heading, canonized as designed intent, both faces kept per framework. |
| A framework's identity — State Stress Test | `State Stress Test` ↔ `STATE STRESS TEST (5 UI States)` | — | Register: chip label ↔ report heading, canonized as designed intent, both faces kept per framework. |
| A framework's identity — Accessibility | `Accessibility` ↔ `ACCESSIBILITY AUDIT` | — | Register: chip label ↔ report heading, canonized as designed intent. Headings are proper nouns — exempt from the audit/report lane split above; "AUDIT" inside a heading is not a violation. |
